...
CONTAINS
  ...
  subroutine NESDIS_SIce_Phy_EM(Frequency4,                                                 &  ! INPUT
                                Angle4,                                                     &  ! INPUT
                                Ts_ice4,                                                    &  ! INPUT
                                Salinity4,                                                  &  ! INPUT
                                Emissivity_H4,                                              &  ! OUTPUT
                                Emissivity_V4)                                                 ! OUPTUT
...
!------------------------------------------------------------------------------------------------------------
     ...
     real(4) ::  Frequency4,Angle4,Ts_ice4,Salinity4,Emissivity_H4,Emissivity_V4   

     real(8) ::  Angle,theta, Ts_ice, Salinity
     real(8) ::  Frequency, Emissivity_H, Emissivity_V, slam, f1,epsx, epsy, pi
     integer(4) :: ITYPE
     COMPLEX(8) :: eair, eice
     COMPLEX(8) :: cos_theta_i,cos_theta_t,sin_theta_i,sin_theta_t
     COMPLEX(8) :: rv_ice, rh_ice
      ...
      theta = Angle*pi/180.0d0

      slam = 300.0d0/Frequency

      f1=Frequency*1.0d9

      eair = dcmplx(1.0d0, 0.0d0)

      ITYPE = 2

      call  permitivity(ITYPE,Ts_ice, Salinity, f1, epsx, epsy)

      eice = dcmplx(epsx, -epsy)

      sin_theta_i = sin(dcmplx(theta,0.0d0))

      cos_theta_i = sqrt(1.0d0 - sin_theta_i*sin_theta_i)

      sin_theta_t = sin_theta_i*sqrt(eair)/sqrt(eice)

      cos_theta_t = sqrt(1.0d0 - sin_theta_t*sin_theta_t)

      call reflection_coefficient(eair, eice, cos_theta_i, cos_theta_t, rv_ice, rh_ice)

      Emissivity_V = 1.0d0  - abs(rv_ice)*abs(rv_ice)

      Emissivity_H = 1.0d0  - abs(rh_ice)*abs(rh_ice)

      Emissivity_H4 = Emissivity_H
      Emissivity_V4 = Emissivity_V
...
!------------------------------------------------------------------------------
     implicit none
     real(8) ::  eo, esw, eswo, a, tswo, tsw, b, sswo, c,d, fi, ssw
     real(8) ::  temp,t, t2, t3, s2, s3, epsp, epspp
     real(8) ::  ESWI, EW0, TPTW, FH, X, Y, TPTWFQ
     real(8) ::  e_real, e_img

     real(8) ::  s,t4,t5,sigma
     real(8) ::  ebo,eb,sb,nb,eb_real,eb_imag
     real(8) ::   den_ice,den_sice,den_brine

     real(8) ::   vss, cc, den_ss

     real(8) ::   f,pi,vb,sigmao,vi

     integer(4) :: itype

     COMPLEX(8) :: ebrine,eice,eair,esice,xx

     pi=acos(-1.0d0)
     ESWI=4.9d0
     t=temp-273.15d0
     if (t.le.-65.0d0) t = -65.0d0
     t2=t*t
     t3=t2*t
     t4=t3*t
     t5=t4*t
...
Case (2) ! sea ice
      ...
      eice = cmplx(3.15d0, -0.001d0,8)
      eair = cmplx(1.0d0, 0.0d0,8)
...
! Normality of brine solution: nb
      ...
      ebrine=cmplx(eb_real,-eb_imag,8)
...
!-----------------------------------------------------------------------------------------------
    ...
    COMPLEX( 8 ) :: rh, rv,cos_theta_i,cos_theta_t

    COMPLEX( 8 ) :: em1, em2, m1, m2
