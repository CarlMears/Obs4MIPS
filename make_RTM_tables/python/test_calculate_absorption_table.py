...
for ioxy in [4,5]:
    ioxy = int(ioxy)
    for msu_channel in [2,3,4]:
        msu_channel = int(msu_channel)
        print(f"Computing absorption tables for MSU channel {msu_channel} with ivap={ivap} and ioxy={ioxy}...")
        abs_table, abs_table_per_Pa = compute_absorption_tables_amsu.make_absorption_table_t_q.compute_absorption_tables_msu(
            msu_channel, ivap, ioxy,
            num_T, num_p, num_q,
        ...
        abs_table_ds = xr.Dataset(
            ...
            attrs={"MSU_channel": msu_channel, "ivap": ivap, "ioxy": ioxy}
        )
        nc_file = f"msu_{msu_channel:02d}_abs_table_q.{ivap}.{ioxy}.nc"
        ...
        nc_file_per_Pa = f"msu_{msu_channel:02d}_abs_table_per_Pa_q.{ivap}.{ioxy}.nc"
